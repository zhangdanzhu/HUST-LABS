...
START:	MOV	AX, DATA
	...
	XOR	AX, AX	;表示开始计时
	CALL	TIMER
LOOP1:	LEA	ECX, BUF[11]
AVE:	XOR	EAX, EAX
	XOR	EDX, EDX
	MOV	AL, BYTE PTR [ECX-1]	;AL中放语文成绩
	MOV	DL, BYTE PTR [ECX]	;DL中放数学成绩
	ADD	ECX, 0EH		;下一学生
	LEA	EAX, [EDX+EAX*2]	
	XOR	EDX, EDX
	MOV	DL, BYTE PTR [ECX-0DH]	;DL中放英语成绩
	LEA	ESI, [EDX+EAX*2]
	MOV	EAX, 92492493H
	IMUL	ESI
	ADD	EDX, ESI
	...
	SHR	EAX, 1FH
	ADD	EDX, EAX
	CMP	ECX, OFFSET BUF[14000]
	MOV	BYTE PTR [ECX-0CH], DL
	JL	AVE
